...
void FluidSim::initialize_mparticles(float (*phi)(const Vec3f&)) {
	int seed = 0;
   ...
   for(int k = 0; k < nk+1; ++k)
		for(int j = 0; j < nj+1; ++j)
			for(int i = 0; i < ni+1; ++i) {
				...
				corner[2] = interpolate_value(Vec3f(i*dx,(j+1)*dx,k*dx), levelset_phi);
				corner[3] = interpolate_value(Vec3f(i*dx,(j+1)*dx,(k+1)*dx), levelset_phi);
				corner[4] = interpolate_value(Vec3f((i+1)*dx,j*dx,k*dx), levelset_phi);
				corner[5] = interpolate_value(Vec3f((i+1)*dx,j*dx,(k+1)*dx), levelset_phi);
				corner[6] = interpolate_value(Vec3f((i+1)*dx,(j+1)*dx,k*dx), levelset_phi);
				corner[7] = interpolate_value(Vec3f((i+1)*dx,(j+1)*dx,(k+1)*dx), levelset_phi);
...
void FluidSim::advect_mparticles(float dt) { 
